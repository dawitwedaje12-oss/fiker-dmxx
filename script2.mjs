...
async function getTikTokThumb(url) {
  try {
    ...
    console.error('TikTok oembed error:', url, e.message);
    return null;
  }
}
...
async function getIgThumb(url) {
  try {
    const res = await fetch(`https://graph.facebook.com/v18.0/instagram_oembed?url=${encodeURIComponent(url)}&access_token=12345|67890`);
    const data = await res.json();
    return data.thumbnail_url;
  } catch (e) {
  }
...
async function run() {
  ...
  // For youtube, let's just add direct URLs so we dont base64 them
  ...
  for (const [key, link] of Object.entries(links)) {
    ...
    console.log(key, 'fetching:', imgUrl);
    try {
      ...
      // if it fetched HTML by accident, skip
      if (ct.includes('html')) throw new Error("fetched html instead of image");
      result[key] = `data:${ct};base64,${b64}`;
    } catch(e) {
      console.log('failed to base64', key, e.message);
  ...
  console.log('done writing cleanly');
}
